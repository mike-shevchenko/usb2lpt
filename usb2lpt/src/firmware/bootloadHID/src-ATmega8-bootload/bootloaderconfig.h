...
// The bootloader condition is software-only.
static inline void  bootLoaderInit(void) {
 unsigned char byte0=pgm_read_byte(0);
 PORTB |= 1<<2;					// STROBE weak HIGH
 if ((unsigned char)(byte0+1) < 2) return;	// without firmware, stay in bootloader
 if (eeprom_read_byte(0) == 0x42) return;	// with this magic number, stay in bootloader
 leaveBootloader();
}

// The bootloader never exits except with -r command line option
